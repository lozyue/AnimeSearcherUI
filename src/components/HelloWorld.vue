            ...
            <div class="helloPanel">
              ...
              <v-card max-width="100%" class="mt-8 helloPanelItem">
                ...
                <v-card-text>
                  ...
                  <div><span>需要间接访问：</span><a target="_blank" href="http://m-p.sakura.ne.jp/">http://m-p.sakura.ne.jp/</a></div>
                </v-card-text>
              </v-card>
              <v-card max-width="100%" class="mt-8 helloPanelItem">
                ...
                <v-card-text style="max-height:300px;overflow-y:scroll" class="lzyalign-left">
                  <div class="">仅当在搜索结果页<b>点击动漫标题</b>的在当前页加载的成功搜索会产生记录</div>
                  ...
                  <div>进入视频详情页会加载弹幕库数据，过快点击播放视频可能导致弹幕库加载失败，请手动辅助选择</div>
...
export default {
  ...
  methods:{
    // 目前调整appbar上的全局开关设置在helloworld页面的快捷分类不会更新
    ...
    queuePostOptions:async function(category, on = true){
      ...
      async function differencePost(categoryToShow, on = true){
        ...
        console.log('差分数据：',danmakuDifferent);
  ...
  computed:{
    // 做设置项分类：
    // showCategoriedSwitches:{
    //   // [视频源/弹幕源, 分类名称]根据参数提供不同分类的设置
    //   get:function(){
    //     return this.switchDanmaku;
    //   },
    //   set:function(newVal){
    ...
    // },
    // comicControl:{
    //   get:function(){return this.categoryControls.comic},
    //   set:function(newVal){
    //     this.queuePostOptions('comic', !!newVal);
    //   }
    // },
    // movieControl:{
    //   get:function(){return this.categoryControls.movie},
    //   set:function(newVal){
    //     this.queuePostOptions('movie', !!newVal);
    //   }
    // },
    // soupOperaControl:{
    //   get:function(){return this.categoryControls.soupOpera},
    //   set:function(newVal){
    //     this.queuePostOptions('soupOpera', !!newVal);
    //   }
    // },
    // performanceControl:{
    //   get:function(){return this.categoryControls.performance},
    //   set:function(newVal){
    //     this.queuePostOptions('performance', !!newVal);
    //   }
    // },
    getRandomColor:{
      get:function(){
