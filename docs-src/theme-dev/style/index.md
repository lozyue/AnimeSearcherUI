...
同时，主题风格的嵌入JavaScript也是当前控制AuiPlayer播放器、扩展应用功能的最好途径。
...
尽管主题配色能够对全局色彩进行调配，但其所能触及的范围还是非常有限。对于以各个配色为基础组合而成的色彩模式，主题配色对此无能为力。

因此通过提供在主题风格中允许CSS样式表，以对色彩模式的修改提供支持。

- [色彩模式修改](style-ref#色彩模式修改)

由于主题风格支持样式表，因此可以根据规范使用CSS类对全局各种样式进行修改，包括圆角、阴影等装饰，甚至是布局（由于AnimeUI对全端做了适配，因此布局模式比较固定可能较难修改）

此外，主题风格设计的宗旨是开放包容的，所以自然允许使用JavaScript对AnimeUI进行修改。并且主题风格还导出了主题风格功能库API、静态函数辅助、以及页面路由和Vue3 Hooks等对象允许开发人员便捷而自由地进行各种添加修改。
同时，使用功能库API对应用进行各种深度定制也是主题风格的任务之一。

综上，主题风格的出现是伴随着AnimeUI的开发而水到渠成的。想要探索主题风格的神奇，就请继续往下阅读吧!
...
```json
{
  ...
  "home": "", // 主题风格介绍页，可选
  "thumbs": "" , // 缩略图链接，可选 可以是路径字符串或者路径字符串数组，顺序靠前的为主缩略图
  "introduce": "", // 主题介绍页面Markdown文件路径，指向一个Markdown格式的.md文件
